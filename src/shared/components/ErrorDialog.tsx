...
const formatValidationError = (errorDetail: { field?: string; message: string; line?: number }) => {
  const fieldPart = errorDetail.field ? `${errorDetail.field}: ` : ""
  const linePart = typeof errorDetail.line === "number" ? ` (linha ${errorDetail.line})` : ""
  return `${fieldPart}${errorDetail.message}${linePart}`
}

export const ErrorDialog = ({ visible, error, onHide }: ErrorDialogProps) => {
  const hasStringDetails = !!error?.errors?.length
  const hasValidationDetails = !!error?.validationErrors?.length
  const hasDetails = hasStringDetails || hasValidationDetails
      ...
      <div className="error-dialog__body">
        ...
        {hasDetails && (
          <div className="error-dialog__details" role="list" aria-label="Detalhes do erro">
            {error.validationErrors?.map((errorDetail, index) => (
              <p
                className="error-dialog__detail-item"
                key={`${errorDetail.field ?? "field"}-${errorDetail.message}-${errorDetail.line ?? "line"}-${index}`}
                role="listitem"
              >
